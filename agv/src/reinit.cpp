...
//  1.1) Subscribe to /amcl_pose for the latest pose. Keep saving the latest pose to file, as the buggy moves. 
...
Reinit::Reinit()
{
  ...
  ROS_ASSERT(private_nh.getParam("amcl_pose_topic", amcl_pose_topic));
  ...
  amcl_pose_sub = nh.subscribe(amcl_pose_topic, 1, &Reinit::amclPoseCallback, this);

  //publish initial pose.
  initialPoseInit(file_name);
}

//Here we process messages from /amcl_pose. Just save to text file.
void Reinit::amclPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& amcl_pose)
{
  //get data from msg
  double position_x = amcl_pose->pose.pose.position.x;
  double position_y = amcl_pose->pose.pose.position.y;
  double position_z = amcl_pose->pose.pose.position.z;
  double quaternion_x = amcl_pose->pose.pose.orientation.x;
  double quaternion_y = amcl_pose->pose.pose.orientation.y;
  double quaternion_z = amcl_pose->pose.pose.orientation.z;
  double quaternion_w = amcl_pose->pose.pose.orientation.w;
  ...
  if (out_file.is_open())
  {
    out_file
      ...
      ;
    out_file.close();
    //ROS_INFO_STREAM("Reinit: wrote an amcl_pose to: " + fullDir);
  }
  else 
  {
...
void Reinit::initialPoseInit (std::string file_name) 
{
  ...
  if (in_file.is_open())
  {
    ...
    ROS_INFO_STREAM("Reinit: read the amcl_pose in: " + fullDir);
    in_file.close();
  }
  else 
  {
